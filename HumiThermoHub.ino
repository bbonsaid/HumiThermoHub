...
typedef struct {
  ...
  char      mac[6];     // mac
  // Byte:8-9 ?
  uint16_t data1;       // 2025.08.21追加 ChatGPTに教えてもらう。
  // Byte:10
  uint8_t decimalOfTheTemperature : 4;
  uint8_t humidityAlertStatus : 2;    //?
  uint8_t temperatureAlertStatus : 2; //?
...
// MQTT設定
const char* mqtt_server = "0.0.0.0";  // MQTTブローカのIPまたはホスト名
const int   mqtt_port   = 1883;
const char* mqtt_user   = "";      // 必要なければ空でOK
...
const unsigned long RESET_INTERVAL = 24UL * 60UL * 60UL * 1000UL; // 24時間(ミリ秒)
...
//    Serial.printf("AD:%s\n", advertisedDevice.toString().c_str());
    ...
    if (advertisedDevice.haveServiceData()) {
      String d = advertisedDevice.getServiceData();
      Serial.printf("SD(%d):", d.length());
    ...
    if (advertisedDevice.haveServiceData()
      ...
      memcpy(&sd, advertisedDevice.getServiceData().c_str(),
      ...
      if (sd.deviceType == 'T' || sd.deviceType == 'i') { // SwitchBot MeterTH or MeterPlus
        ...
        if (pos >= 0 && fConnect) {
          ...
          // MQTT送信
          char payload[256];
          snprintf(payload, sizeof(payload), "{\"address\":\"%s\",\"temperature\":%.1f,\"humidity\":%.1f}",
                  address.toString().c_str(), temperature, humidity);
...
*/
    ...
    if (advertisedDevice.haveManufacturerData() == true
      ...
      memcpy(&md, advertisedDevice.getManufacturerData().c_str(),
          advertisedDevice.getManufacturerData().length());

      if (md.companyID == 0x0969 && md.data1 == 0x0ec8) {
        ...
        Serial.printf("md.data1:%04x\n", md.data1);

        Serial.printf("S temp:%.1f\n", temperature);
        Serial.printf("S humi:%.1f\n", humidity);
    ...
    if (advertisedDevice.haveManufacturerData() == true
      ...
      if (md.companyID == 0x004C && md.deviceType == 0x02) {
        ...
        Serial.printf("C temp:%.1f\n", temperature);
        Serial.printf("C humi:%.1f\n", humidity);
...
void setup() {
  ...
  for (int i = 0; i < 100 && WiFi.status() != WL_CONNECTED; i++) {  //  Wi-Fi AP接続待ち
    Serial.print(".");
    delay(1000);
  }
  Serial.print("\n");
...
//  printAddressPos();
  ...
  if (millis() - lastResetMillis >= RESET_INTERVAL) {
    Serial.println("24時間経過。ESP32をリセットします。");
    ESP.restart();
  }
